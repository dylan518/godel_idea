...
- Consistent deepseek-chat blind on every trajectory transition
  - `blind_S_paper_vs_S12__deepseek-chat.json` (N=75, win_rate_b=0.807; reboot baseline, `S_sota.py`)
  - `blind_S12_vs_S14__deepseek-chat.json` (N=75, win_rate_b≈0.507)
  - `blind_S12_vs_S15__deepseek-chat.json` (N=75, win_rate_b=0.573)
...
- Multi-judge agreement on S15 vs S_paper (publish-eval bundle with **75** on-disk pair slots; headline **64**-pair intersection across judges)
  - Script: `ideas/compute_multi_judge_agreement.py` (inputs: `judge_deepseek-chat.json`, `judge_gpt-5.4.json`, `judge_gemini-3-flash-preview.json` under `publish_eval_n75_gemini3_deepseek_S15_Spaper/`)
  - Judges: gemini-3-flash-preview, gpt-5.4, deepseek-chat
  - Output: `ideas/results/multi_judge_agreement_S15_vs_Spaper.json` (30-pair legacy: `_archive_multi_judge_S15_vs_Spaper_n30_20260419.json`)
...
- `compare_S12_vs_S15.json` primary archive vs blind DeepSeek agreement is summarized in `judge_agreement_deepseek_blind.json` (κ updates whenever the blind file is regenerated).
...
| -------------- | ------- | ---------- | ------------ | ------------- | ----------------------- | ----------------------------------------- |
| S_paper → S12  | 75      | 0.807      | 0.72         | 0.887         | —                       | blind only (`S_sota.py` side A)           |
| S12 → S14      | 75      | 0.507      | 0.38         | 0.627         | 0.324                   | rejected candidate; primary+blind both DS |
| S12 → S15      | 75      | 0.573      | 0.493        | 0.667         | 0.171                   | see `trajectory_table.csv`                |
...
**Version numbering:** the **accepted** spine in `evolution_log.jsonl` is `**S_paper` → S12 → S15** (log label for reboot baseline = `S_sota.py`; then a separate `**S15` → `S_paper`** branch for `S_paper.py`). **S14** and **S16–S19** are *rejected challengers* (vs S12 or S15), not sequential replacements. **S13** is a generator-only line in this narrative unless you add compares.
...
| ------------------------------ | --------------- | --------------------------------------------- | ----------------------------------------------- |
| **S_paper (reboot), S12, S15** | yes             | S12, S15 yes                                  | Champion path                                   |
| **S_paper**                    | yes             | under `publish_eval`_* + top-level `S_paper`  | Paper-aligned comparator                        |
...
**Primary human evidence for the workshop write-up:** the **CUSTOM slice** (same topics as the UI session, regenerated ideas + multi-judge replay). Same-text T1–T15 fixed benchmark is **optional** (only if you want pair-level alignment with `publish_eval`_* idea texts).

- **CUSTOM slice (recommended):** existing UI log → `ideas/results/human_blind_scores.jsonl` (S15 vs S_paper). Regenerate/judge with `ideas/run_custom_slice_judge.py` → `ideas/results/custom_slice_human_vs_judge/summary.json`. Metrics: `ideas/compute_human_vs_judge.py` → `ideas/results/custom_slice_human_vs_judge/human_vs_judge.json`.
...
- **Fixed T1–T15 benchmark (optional):** run `ideas/prepare_fixed_benchmark_human_eval.py`, then `human_judge_ui.py` with `?left=fixed_benchmark_S15&right=fixed_benchmark_S_paper` (30 slots, identical texts to `publish_eval_20260413T163806Z`). Pair-level judge alignment: `ideas/compute_human_judge_agreement_fixed.py` → e.g. `results/human_judge_agreement_fixed.json`.

Caveats (document in paper):

- Single rater → kappa between humans not estimable; pilot is descriptive only.
- Human ratings were on on-the-fly-generated ideas that were not persisted, so judge-human agreement on the CUSTOM path is **topic-level** (different idea instantiations, same systems/topics), not pair-level.
...
- **Qualitative “what changed / why judges split”** (for Discussion): run `ideas/summarize_workshop_qualitative.py` (one LLM call per transition; reads `compare_*.json` `reasoning` fields + both `systems/S*.py`). Example: `cd ideas && python3 summarize_workshop_qualitative.py --batch` → `results/workshop_qualitative/*.md`. Edit lightly for the paper; keep caveats that rationales are post-hoc.
...
- Discussion bullets from `summarize_workshop_qualitative.py` outputs (generator delta + judge-rationale themes)
