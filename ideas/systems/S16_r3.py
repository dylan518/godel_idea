"""S16_r3: Hypothesis-First Adversarial Loop with Mechanism-Anchored Construction.
...
3. HYPOTHESIS SELECTION (1 call): Aggregate attack+revision pairs and pick the
   ...
   concrete, novel, and falsifiable. Also extracts the CAUSAL MECHANISM that
   makes the hypothesis non-trivial.

4. IDEA CONSTRUCTION (1 call): Design the full experimental idea around
   proving/disproving the surviving hypothesis. Anchored to the explicit causal
   mechanism — experimental design must differ if mechanism is wrong.
...
class S16_r3Generator(IdeaGenerator):
    VERSION = "S16_r3"
    DESCRIPTION = (
        "Hypothesis-first adversarial loop with mechanism-anchored construction: "
        "generate falsifiable hypotheses, attack each in parallel, select strongest "
        "survivor while extracting causal mechanism, build experimental idea anchored "
        "to that mechanism (not just hypothesis label), then multi-perspective critique."
    )
    ...
    def generate_idea(
        ...
        # ── Step 3: select the strongest surviving hypothesis + extract mechanism ──
        pairs_str = ""
        ...
        select_prompt = (
            ...
            "CAUSAL MECHANISM: <1-3 sentences explaining WHY this hypothesis is true if correct — "
            "name the specific mechanism (e.g., 'disentangling X from Y reduces interference because Z'), "
            "NOT just restating the outcome. This must be load-bearing: the experimental design "
            "should be different if this mechanism is wrong.>\n"
            "MECHANISM ABLATION: <name ONE concrete ablation that isolates the mechanism — "
            "a variant that removes only the proposed mechanism while keeping everything else>\n"
            "REASONING: <1-2 sentences on why this is the strongest>"
        )
        try:
            selection_raw = call_llm(select_prompt, model, client, temperature=0.3)
        except Exception as e:
            selection_raw = f"SELECTED: 1\nREVISED HYPOTHESIS: {hypotheses[0]}\nCAUSAL MECHANISM: The mechanism is unknown.\nMECHANISM ABLATION: Remove the proposed component.\nREASONING: fallback"

        # Extract the selected revised hypothesis, mechanism, and ablation
        ...
        for line in selection_raw.strip().split("\n"):
            ...
            elif stripped.upper().startswith("CAUSAL MECHANISM:"):
                causal_mechanism = stripped.split(":", 1)[1].strip()
            elif stripped.upper().startswith("MECHANISM ABLATION:"):
                mechanism_ablation = stripped.split(":", 1)[1].strip()

        # ── Step 4: construct experimental idea anchored to the mechanism ────
        context_reminder = (
        ...
        if causal_mechanism:
            mechanism_block = (
                f"\nCausal mechanism to anchor the design: {causal_mechanism}\n"
                f"Mechanism ablation to include: {mechanism_ablation}\n"
            )
        ...
        construct_prompt = (
            ...
            "CRITICAL REQUIREMENT: The experimental design must be ANCHORED TO THE CAUSAL MECHANISM above.\n"
            "- First, state in 1-2 sentences WHY the mechanism produces the predicted effect "
            "(theoretical grounding — cite a principle, not just restate the hypothesis).\n"
            "- Then design experiments where the MECHANISM IS LOAD-BEARING: the experimental "
            "design must be different if the mechanism is wrong (not just the outcome).\n"
            "- Include the mechanism ablation as a mandatory baseline — this is the key control "
            "that distinguishes your mechanism from alternative explanations.\n"
            ...
            "Write 3-4 paragraphs. Be direct and specific. The mechanism must appear in paragraph 1."
        )
        try:
        ...
        if causal_mechanism:
            mechanism_reminder = (
                f"\nIMPORTANT: Preserve the causal mechanism in the final version: {causal_mechanism}\n"
                "The mechanism must be explicitly named in the IDEA section and the experimental "
                "design must include the mechanism ablation as a baseline.\n"
            )
        ...
        revise_prompt = (
            ...
            f"{context_reminder}\n"
            f"{mechanism_reminder}\n"
            "Write the final, improved version of the research idea. "
            "Ensure the hypothesis is clearly stated, the causal mechanism is explicitly named, "
            "the falsification experiment is concrete, "
